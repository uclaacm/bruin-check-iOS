...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    // Used to force the labels to be positioned on top of the graph
    @IBInspectable open var dataPointLabelOnTop: Bool = false
    ...
    private func updateLabels(deactivatedPoints: [Int], activatedPoints: [Int]) {
        ...
        for point in activatedPoints {
            ...
            label.frame = CGRect(origin: CGPoint(x: position.x - label.frame.width / 2, y: (dataPointLabelOnTop ? 0 : position.y) + dataPointLabelTopMargin), size: label.frame.size)
